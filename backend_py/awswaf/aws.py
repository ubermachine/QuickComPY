...
class AwsWaf:
    def __init__(self, goku_props: dict, endpoint: str, domain: str,
    ...
    def build_payload(self, inputs: dict):
        solver = CHALLENGE_SOLVERS[inputs["challenge_type"]]
        checksum, fp = get_fp(self.user_agent)
        return {
            "challenge": inputs["challenge"],
            "checksum": checksum,
            "solution": solver(inputs["challenge"]["input"], checksum, inputs["difficulty"]),
    ...
    def verify(self, payload):
        url = f"https://{self.endpoint}/verify"
